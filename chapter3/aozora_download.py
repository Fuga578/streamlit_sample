from io import BytesIO
import re
import sys
import zipfile
import requests


def get_page(url: str) -> bytes:
    """zipファイルデータを取得します。
    
    Args:
        url (str): URLまたはzipファイル名
    
    Returns:
        bytes: zipバイナリデータ
    """

    # URLではない場合
    if not url.startswith("http"):
        with open(url, "rb") as fp:
            return fp.read()
        raise Exception(f"Failed to read local file: {url}")

    response = requests.get(url)
    if response.status_code != 200:
        raise Exception(f"Http failure. Code {response.status_code} {response.text}")
    
    return response.content


def parse_zipped(zipped_data: bytes) -> str:
    """zipバイナリデータからテキストを取得します。
    
    Args:
        zipped_data (bytes): zipバイナリデータ
    
    Returns:
        str: テキストデータ
    """

    # 読み込み
    zp = zipfile.ZipFile(BytesIO(zipped_data))

    # txtファイルオブジェクトのみ抽出
    text_files = [z for z in zp.infolist() if z.filename.lower().endswith(".txt")]
    if len(text_files) == 0:
        raise Exception("Can't find any text file in the Zip.")
    info = text_files[0]    # 先頭1件のみ抽出

    with zp.open(info) as fp:
        text_bytes = fp.read()
        text = text_bytes.decode("shift-jis", errors="replace")

    return text


def sanitize_aozora(text: str) -> str:
    """青空文庫の文章データから、余分なデータを削除します。
    
    Args:
        text (str): 青空文庫の文章
    
    Returns:
        str: 余分なデータを削除した青空文庫の文章
    """

    # --- (55個) で囲まれた分はメタデータなので抜く
    first = text.find("-"*55, 0)
    second = text.find("-"*55, first+55)
    text = text[:first] + text[second+55:]

    # ［］（全角角カッコ）は注釈なので抜く
    text = re.sub(r'［.+?］', '', text)

    # 《》はルビなので抜く。｜（全角縦棒）もルビの範囲を示す記号なので抜く。
    text = re.sub(r'《.+?》', '', text)
    text = text.replace('｜', '')

    # 最後の底本情報も抜く。
    last = text.rfind('\r\n\r\n')
    text = text[:last]

    return text


def get_aozora_text(url: str) -> str:
    """青空文庫の指定URLのテキストを取得します。
    
    Args:
        url (str): URLまたはzipファイル名
    
    Returns:
        str: テキスト
    """

    zipped_data = get_page(url)
    text = parse_zipped(zipped_data)
    sanitized = sanitize_aozora(text)

    return sanitized


def parse_text_into_sentences(text: str) -> list[str]:
    """テキストを文章に変換します。
    
    Args:
        text (str): テキスト
    
    Returns:
        list[str]: 文章リスト
    """
    paragraphs = [par for par in re.split(r'[\r\n]+', text) if len(par) > 0]
    sentences = []
    for par in paragraphs:
        sentences.extend([s.strip() for s in re.split(r'。[」』]*', par) if len(s) > 0])

    return sentences


if __name__ == '__main__':
    import sys
    url = sys.argv[1]
    text = get_aozora_text(url)
    print(text)

    sentences = parse_text_into_sentences(text)
    print('\n'.join(sentences))

